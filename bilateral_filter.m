...
%
%    bilateral_filter(A, W, SIGMA) performs 2-D bilateral filtering
%    for the grayscale image A. A should be a double precision
%    matrix of size NxMx1 (i.e., grayscale) with normalized values in
...
for i = 1:dim(1)
   for j = 1:dim(2)
         % if A(i,j)>= 0    % only operate on edges
         if ~isnan(A(i,j))  % only operate on edges (MOD PV)
         ...
         % B(i,j) = nansum(F(:).*I(:))/nansum(F(:));  
         num = F(:).*I(:); num = sum(num(~isnan(num)));     % MOD PV
         denom = F(:); denom = sum(denom(~isnan(denom)));   % MOD PV
         B(i,j) = num/denom;                                % MOD PV
         end
   end
   waitbar(i/dim(1));
end
